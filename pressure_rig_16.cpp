...
int main ()
{
    ...
    const uint sck_pin = 2;   //GP6 SCLK 
    const uint mosi_pin = 7;  //GP7 MOSI TX 
    const uint miso_pin = 4; // MISO RX
    uint cs_pin = 1;  //GP5 
    const uint cs_pin_dac = 8; //GP8 can be made into a chip select
    ...
    // Pot stuff should be commented out **************************************************
    adc_init();
    adc_gpio_init(28);
    adc_select_input(2);
    uint16_t pot_val;
    ...
    while(1)
    {
        send_pressure(0x255,cs_pin_dac,spi,value);
    }
